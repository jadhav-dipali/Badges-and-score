...
const nPages = Math.ceil(data.length / recordsPerPage)
    ...
    <table id="tabel-user">
        ...
        {currentRecords.map((info)=>{
            return <>
                <tr>
                <td id="select-column"> <input type="checkbox"></input></td>
                <td>{info.name}</td>
                <td>{info.email}</td>
                <td>{info.role}</td>
                <td id="icon-column">
                <img id="edit-icon" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRIcnFzEj5VainEnRAY2Y4P9UgZzfnx4aV7GHs-w7lSwiYSj6Rcc8t6NpTgFvYb4SH58dc&usqp=CAU"/>
